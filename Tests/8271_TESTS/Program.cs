...
for (int i = 0; i < args.Length; i++)
{
    ...
    {
        Console.Error.WriteLine("Usage: 8271_TESTS [--list] [--filter name]");
        return 2;
    }
}

var tests = new List<(string Name, Action Body)>();
ControllerTests.Add(tests);
SectorTests.Add(tests);
ResultTests.Add(tests);
Console.WriteLine("Provenance: SourcedBBC = adapted upstream BBC tests; Unverified = no established hardware result. See PROVENANCE.md.");
