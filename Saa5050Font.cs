// ============================================================================
// Project:     BBC
// File:        Saa5050Font.cs
// Description: Mullard SAA5050 teletext alphanumeric glyph renderer.
// Author:      James Booth
// Created:     2026
// License:     MIT License - See LICENSE file in the project root
// Copyright:   (c) 2024-2026 James Booth
// Notice:      BBC Micro ROMs are property of their respective rights holders.
//              SAA5050 glyph data adapted from textmodes/font, MIT licensed.
// ============================================================================

namespace BBC
{
    /// <summary>
    /// Draws the SAA5050 English teletext alphanumeric character set used by BBC Micro MODE 7.
    /// </summary>
    internal static class Saa5050Font
    {
        private const int GlyphWidth = 5;
        private const int GlyphHeight = 10;
        private const int GlyphXScale = 2;
        private const int GlyphYScale = 2;
        private const int DoubleHeightGlyphYScale = 4;
        private const int GlyphXOffset = 2;
        private const int GlyphYOffset = 0;

        private static readonly byte[] GlyphRows =
        [
            
        ];

        public static void Draw(uint[] pixels, int width, int height, int cellX, int cellY, int cellWidth, int cellHeight, byte character, uint colour, bool doubleHeight, bool doubleHeightBottom)
        {
            character = (byte)(character & 0x7F);
            if (character < 32)
                character = 32;

            int rowScale = doubleHeight ? DoubleHeightGlyphYScale : GlyphYScale;
            int sourceYOffset = doubleHeightBottom ? cellHeight : 0;
            int glyphOffset = (character - 32) * GlyphHeight;

            for (int glyphY = 0; glyphY < GlyphHeight; glyphY++)
            {
                byte bits = GlyphRows[glyphOffset + glyphY];
                for (int glyphX = 0; glyphX < GlyphWidth; glyphX++)
                {
                    if ((bits & (0x10 >> glyphX)) == 0)
                        continue;

                    int pixelX = cellX + GlyphXOffset + (glyphX * GlyphXScale);
                    int pixelY = cellY + GlyphYOffset + (glyphY * rowScale) - sourceYOffset;

                    for (int yy = 0; yy < rowScale; yy++)
                    {
                        int y = pixelY + yy;
                        if (y >= cellY + cellHeight || (uint)y >= (uint)height)
                            continue;

                        int offset = y * width;
                        for (int xx = 0; xx < GlyphXScale; xx++)
                        {
                            int x = pixelX + xx;
                            if ((uint)x < (uint)width)
                                pixels[offset + x] = colour;
                        }
                    }
                }
            }
        }
    }
}
