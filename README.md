...
Save states are written from the File menu using the host file picker and use a `.sav` extension. Successful saves and opens are added to a small recent-state list in the File menu, so commonly used states can be reopened without reopening the file picker. The snapshot includes BBC RAM and device state, serial ACIA and Hayes modem configuration, mounted DFS image state, sideways ROM bank contents and paths, and the 65C02 Tube state when the co-processor is enabled. Live Hayes TCP connections are not restored; a loaded modem returns disconnected in command mode. Save states are tied to the current emulator save-state format; older save-state formats are rejected rather than converted.
...
The Hayes panel appears in a boxed overlay at the bottom centre while the modem is enabled. Its LEDs follow the familiar front-panel names: `HS`, `AA`, `CD`, `OH`, `RD`, `SD`, `TR`, and `MR`. `HS` and `TR` stay lit while the modem is active, `OH` lights during the ATDT dial, ringback, handshake, and connected phases, `CD` lights only after the TCP connection is established, `RD` and `SD` flash for modem-to-BBC and BBC-to-modem data, and `MR` shows that the modem object is active. The Hayes drop-down menu opens above the panel and provides `Loopback` and `Reset`; reset drops any connection, clears loopback and command state, and returns the modem to command mode.

The Hayes modem handles command mode and TCP-backed connected mode. `ATDThost:port` resolves the target to an IPv4 address, plays a one-second dial tone and fast SN76489-approximated DTMF digits derived from that IPv4 address, verifies the TCP port with a short probe socket, plays a compressed post-dial silence, a UK-style ringback double ring, and a modem answer/training sequence, then opens the real host TCP connection, defaulting to port 23 when no port is supplied. `ATDPhost:port` is accepted as the pulse-dial equivalent. A successful dial returns `CONNECT host port`; connection failure returns `NO CARRIER`, while a missing host network returns `NO DIALTONE`. `ATDhost:port`, `ATDT host:port`, and space-separated `ATDhost port` forms are rejected because the dial command must include the tone or pulse modifier immediately before the target. `ATH` hangs up, `ATZ` resets the command state and hangs up, `ATO` returns to online mode after escaping, and `+++` escapes from connected mode back to command mode.
